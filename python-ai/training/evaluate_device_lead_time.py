...
from data.device_failure_dataset import DEVICE_FEATURE_COLUMNS

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model = FailurePredictor(num_features=6).to(device)
model.load_state_dict(
    torch.load(ROOT / "saved_models" / "device_failure_predictor.pth", map_location=device)
)
model.eval()
...
for device_id, group in df.groupby("deviceId"):
    ...
    if first_alert_time is not None:
        ...
        print(f"Device {device_id}: alerted at t={first_alert_time:.2f}, "
              f"failed at t={failure_time:.2f}, lead={lead:.2f}s")
    else:
        print(f"Device {device_id}: no early alert before failure at t={failure_time:.2f}")

if lead_times:
    print(f"\nAverage device prediction lead time: {np.mean(lead_times):.2f}s over {len(lead_times)} failures")
else:
